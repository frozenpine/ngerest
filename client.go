...
func (c *APIClient) prepareRequest(
	...
	if ctx != nil {
		...
		if auth, ok := ctx.Value(ContextAPIKey).(APIKey); ok {
			expire := int(time.Now().Unix()) + 5
			localVarRequest.Header.Add("api-expires", strconv.Itoa(expire))

			signature := c.generateSignature(auth.Secret, method, url, expire, body)
			localVarRequest.Header.Add("api-signature", signature)
		}
	}
